...
dss=dss(1+2*(Nx+5):end);
dnn(1+2*(Nx+5):end)=dnn(1:end-2*(Nx+5));



A=spdiags(dmmm,-3,Z)+spdiags(dmm,-2,Z)+spdiags(dm,-1,Z)+spdiags(d0,0,Z)+spdiags(dp,1,Z)+spdiags(dpp,2,Z)+spdiags(dppp,3,Z); 

%add in 2nd order y derivatives

A=A+spdiags(dnn,2*(Nx+5),Z)+spdiags(dss,-2*(Nx+5),Z);
